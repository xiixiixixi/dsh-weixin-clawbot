...
import { resolveMediaDir } from './config.js';
import { HELP_TEXT, chunkForWechat, extractAssistantText, mediaPlaceholder, parseControlCommand, turnEndSummary, } from './format.js';
import { IlinkChannel } from './ilink.js';
import { decideDm, decideGroup } from './policy.js';
import { granularityFromConfig, migrateLegacyStateFile, loadState, noticeToolsOf, replyOnOf, resolveStatePath, saveState, } from './state.js';
import { isWechatSession, sessionIdFor, subjectFromSessionId, } from './sessions.js';
...
export class WechatBackend {
    ...
    subjectWorkspace = new Map();
    /** 主体 → 当前会话 id（/new 后换成新生代 id）。 */
    subjectSession = new Map();
    /** /new 标记：下条消息铸造全新会话 id。 */
    freshNext = new Set();
    /** 进程代次后缀：重启后换新 id，避免与磁盘持久化日志撞车。 */
    processGen = `~p${process.pid.toString(36)}${Math.random().toString(36).slice(2, 6)}`;
    constructor(ctx, config, stateFile = resolveStatePath()) {
        this.ctx = ctx;
        this.config = config;
    ...
    allowedWorkspaceIds() {
        ...
        if (scope === 'all')
            return existing.map((workspace) => workspace.id);
    ...
    currentWorkspaceId(subject) {
        const override = this.subjectWorkspace.get(subjectKey(subject));
        if (override !== undefined && this.allowedWorkspaceIds().includes(override))
            return override;
    ...
    async logout() {
        ...
        await this.bot?.logout().catch((error) => this.log(`[wechat] 解绑失败: ${String(error)}`));
        this.currentUser = undefined;
        this.qrState = { kind: 'none' };
    }
    /** 测试可见的 agent 创建入口。 */
    async getOrCreateAgentForTest(sessionId, cwd) {
        return this.getOrCreateAgent(sessionId, cwd);
    }
    /** 异步渲染二维码 PNG（供 Web 扫码窗口内嵌显示）；保留同码上的 verifyCode 标记。 */
    async renderQrPng(qrcode, status) {
        try {
    ...
    /** 二维码自动刷新次数上限。 */
    static ILINK_QR_REFRESH_LIMIT = 5;
    async startIlink() {
        ...
        channel.setBotAgent(typeof this.config.puppetOptions.botAgent === 'string'
    ...
    async runIlinkLoop() {
        ...
        (message) => {
            this.handleIlinkMessage(message).catch((error) => this.log(`[wechat] 处理消息失败: ${String(error)}`));
    ...
    async handleIlinkMessage(message) {
        if (!this.started)
            return;
        const subject = message.groupId !== undefined
    ...
    async dispose() {
        ...
        for (const { dispose } of this.owned.values()) {
            await dispose().catch((error) => this.log(`[wechat] 释放会话失败: ${String(error)}`));
        }
        this.owned.clear();
        this.ilinkChannel?.stop();
        this.ilinkChannel = undefined;
        this.contextTokens.clear();
        await this.bot?.stop().catch((error) => this.log(`[wechat] 停止机器人失败: ${String(error)}`));
        this.bot = undefined;
        this.qrState = { kind: 'none' };
    }
    /** 入站微信消息 → agent。 */
    async handleMessage(message) {
        ...
        if (isGroup) {
            ...
            if (!decision.allowed) {
                if (decision.reason === 'no-mention')
                    this.log(`[wechat] 忽略群消息（未 @机器人）: ${room.id}`);
                return;
            }
        }
        else {
            const decision = decideDm(this.config, { id: from.id, name: from.name() });
            if (!decision.allowed) {
                if (decision.reason === 'pairing') {
    ...
    async dispatchToAgent(subject, body) {
        ...
        this.log(`[wechat] 入站 ${subject.kind === 'group' ? `群 ${subject.id}` : `用户 ${subject.id}`}: ${body.slice(0, 120)}`);
    }
    /** 好友请求处理：按配置自动通过。 */
    async handleFriendship(friendship) {
    ...
    async onSessionEvent(session, event) {
        if (!this.started)
            return;
        if (this.bot === undefined && this.ilinkChannel === undefined)
            return;
        if (!isWechatSession(session.id))
            return;
        ...
        switch (event.type) {
            case 'assistant/message': {
                if (replyOnOf(this.effectiveSettings().granularity) !== 'step')
                    return;
                const text = extractAssistantText(event.data.message);
                if (text.trim())
                    await this.reply(targetId, text);
                return;
            }
            case 'tool/call': {
            ...
            case 'turn/end': {
                ...
                if (summary)
                    await this.reply(targetId, summary);
                return;
            }
            default:
                return;
    ...
    /** 控制命令。 */
    async handleCommand(subject, command) {
        if (this.bot === undefined && this.ilinkChannel === undefined)
            return;
        const targetId = subject.id;
        ...
        switch (command?.kind) {
            case 'new': {
                if (owned) {
                    await owned.dispose().catch((error) => this.log(`[wechat] 释放会话失败: ${String(error)}`));
    ...
    wsCommandReply(subject, arg) {
        ...
        if (Number.isInteger(index) && index >= 1 && index <= rows.length) {
            ...
            const hits = rows.filter((workspace) => workspace.title.toLowerCase().includes(needle)
     ...
     */
    async getOrCreateAgent(sessionId, cwd, workspaceId) {
        ...
        if (workspaceId !== undefined && this.workspaceRegistry !== undefined) {
            const record = this.workspaceRegistry.list().find((workspace) => String(workspace.id) === workspaceId);
            try {
    ...
    async reply(targetId, text) {
        ...
        if (ilink !== undefined) {
            ...
            for (const chunk of chunks) {
                if (chunk.trim()) {
                    await ilink.sendText(targetId, chunk, contextToken).catch((error) => this.log(`[wechat] iLink 发送失败: ${String(error)}`));
                }
            }
            return;
        }
