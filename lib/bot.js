...
export async function resolvePuppetConfig(puppet, puppetOptions) {
    ...
    if (!candidates)
        return { puppet, puppetOptions };
    let mod;
    try {
        mod = await import(/* webpackIgnore: true */ puppet);
    }
    catch (error) {
        throw new Error(`微信接入包未安装：${puppet}。请先 npm i ${puppet}。原始错误：${String(error)}`);
    }
    const moduleRecord = mod;
    for (const name of candidates) {
        const cls = moduleRecord[name] ??
            moduleRecord.default?.[name];
        if (typeof cls === 'function') {
            return {
                puppet: new cls(puppetOptions),
                puppetOptions,
            };
        }
    }
...
export function createWechatBot(opts) {
...
export function mediaKindOf(type) {
    switch (type) {
...
export async function sendToRecipient(bot, targetId, opts) {
    const recipient = (await bot.Contact.find({ id: targetId }).catch(() => undefined)) ??
