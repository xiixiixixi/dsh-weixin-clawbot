 ...
 */
export function chunkForWechat(text, limit) {
...
export function parseControlCommand(text) {
    ...
    if (/^\/new\b/.test(lower) || trimmed === '/新会话')
        return { kind: 'new' };
    if (/^\/stop\b/.test(lower) || trimmed === '/停止')
        return { kind: 'stop' };
    if (/^\/status\b/.test(lower) || trimmed === '/状态')
        return { kind: 'status' };
    if (/^\/help\b/.test(lower) || trimmed === '/帮助')
        return { kind: 'help' };
    const ws = /^\/(?:ws|工作区)\b\s*(.*)$/.exec(trimmed);
    if (ws)
        return { kind: 'ws', arg: ws[1].trim() || undefined };
    return null;
}
...
/** turn 结束原因 → 给用户的错误摘要。 */
export function turnEndSummary(reason) {
    switch (reason.kind) {
        case 'error':
...
/** 入站媒体占位文本（模型可见）。 */
export function mediaPlaceholder(kind, path) {
    const tag = {
        image: 'image',
        voice: 'audio',
        video: 'video',
