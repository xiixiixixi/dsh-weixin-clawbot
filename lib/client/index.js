...
function WechatFooterAction(props) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Tooltip, { label: `微信机器人 · ${status.text}`, side: "right", delayMs: 300, children: _jsxs("button", { type: "button", className: `dsh-weixin-clawbot-entry${props.wide ? ' dsh-weixin-clawbot-entry-wide' : ''}`, "aria-haspopup": "dialog", "aria-expanded": open, onClick: () => setOpen(true), children: [_jsx(PhoneGlyph, {}), props.wide && _jsx("span", { children: "\u5FAE\u4FE1" }), badgeTone !== null && (_jsx("span", { className: `dsh-weixin-clawbot-badge${badgeTone === 'error' ? ' dsh-weixin-clawbot-badge-error' : ''}`, "aria-hidden": "true" }))] }) }), _jsx(WechatDialog, { open: open, onClose: () => setOpen(false), payload: payload, error: error, refresh: refresh })] }));
}
