 ...
 */
export function sessionIdFor(subject, workspaceId) {
...
/** 判断一个会话 id 是否由本插件创建。 */
export function isWechatSession(id) {
    return String(id).startsWith(WECHAT_SESSION_PREFIX);
}
/** 从会话 id 反推微信主体（用于外发时定位收件人）；工作区后缀被剥离。 */
export function subjectFromSessionId(id) {
    const raw = String(id);
    if (!raw.startsWith(WECHAT_SESSION_PREFIX))
        return undefined;
...
export function sendTargetFromSessionId(id) {
