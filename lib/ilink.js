...
export class IlinkChannel {
    ...
    /** 发起新的扫码登录（丢弃旧二维码）。 */
    async startLogin() {
        const body = await this.postJson(ILINK_LOGIN_BASE_URL, 'ilink/bot/get_bot_qrcode?bot_type=3', { local_token_list: this.credentials ? [this.credentials.token] : [] }, { token: false });
        const parsed = body;
    ...
    async pollLoginOnce() {
        ...
        let parsed;
        try {
            parsed = await this.getJson(ILINK_LOGIN_BASE_URL, `ilink/bot/get_qrcode_status?${params.toString()}`, { timeoutMs: 35_000, token: false });
        }
        catch (error) {
            // 网关超时/网络抖动视为继续等待
    ...
    async runMessageLoop(onMessage, onError) {
        const credentials = this.credentials;
        if (!credentials)
            throw new Error('iLink 未登录，无法启动消息循环');
        this.running = true;
        this.log(`[wechat] iLink 消息循环启动（网关: ${credentials.baseUrl}）`);
        // 官方协议要求客户端启动时通知服务端，消息路由才会建立（失败不阻断）
        try {
            const start = await this.postJson(credentials.baseUrl, 'ilink/bot/msg/notifystart', { base_info: { bot_agent: this.botAgentHeaderValue() } }, { token: true, timeoutMs: 15_000 });
        ...
        while (this.running && this.credentials !== undefined) {
            try {
                const response = await this.postJson(credentials.baseUrl, 'ilink/bot/getupdates', { get_updates_buf: this.updatesBuf, base_info: { bot_agent: this.botAgentHeaderValue() } }, { token: true, timeoutMs: 70_000 });
    ...
    stop() {
        if (this.running) {
            const credentials = this.credentials;
            if (credentials) {
                void this.postJson(credentials.baseUrl, 'ilink/bot/msg/notifystop', { base_info: { bot_agent: this.botAgentHeaderValue() } }, { token: true, timeoutMs: 5000 }).catch(() => { });
            }
        }
        this.running = false;
    }
    ...
    async sendText(toUserId, text, contextToken) {
        const credentials = this.credentials;
        if (!credentials)
            throw new Error('iLink 未登录，无法发送消息');
        const response = await this.postJson(credentials.baseUrl, 'ilink/bot/sendmessage', {
            msg: {
                from_user_id: '',
                to_user_id: toUserId,
    ...
    async request(baseUrl, endpoint, init) {
    ...
    async postJson(baseUrl, endpoint, body, opts) {
        return this.request(baseUrl, endpoint, { method: 'POST', body, ...opts });
    }
    async getJson(baseUrl, endpoint, opts) {
...
function normalizeInbound(msg) {
    ...
    for (const item of items) {
        ...
        if (typeof voiceText === 'string' && voiceText !== '')
...
export function loadIlinkCredentials(file, log) {
    try {
...
/** 原子写入凭据。 */
export function saveIlinkCredentials(file, credentials, log) {
